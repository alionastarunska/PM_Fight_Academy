...
class BookingActivityViewController: UIViewController, BookingNewActivity {
    ...
    private func fetchTimeSlots() {
        ...
        PMFightApi.shared.time(for: activity.id,
                               with: coach.id,
                               on: date.stringed(using: .pmDateFormatter)) { [weak self] (result) in
            self?.endLoading()
            switch result {
    ...
    private func createBooking(_ activity: TrainingType, _ coach: Coach, _ date: Date, _ timeSlot: TrainingTime) {
        startLoading()
        PMFightApi.shared.bookActivity(for: activity.id,
                                       with: coach.id,
                                       on: date.stringed(using: .pmDateFormatter),
                                       time: timeSlot) {  [weak self] (result) in
            self?.endLoading()
            switch result {
            case .none:
